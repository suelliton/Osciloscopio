...
void setup(){
        ...
        InLabelA0 = controlP5.addTextlabel("In","1",65,205);  
        InLabelA1 = controlP5.addTextlabel("In2","1",65,240);
        InLabelA2 = controlP5.addTextlabel("In3","1",65,275);
        InLabelA3 = controlP5.addTextlabel("In4","1",65,310);
        InLabelA4 = controlP5.addTextlabel("In5","1",65,345);
        InLabelA5 = controlP5.addTextlabel("In6","1",65,380);
        
        QuadroLabel =controlP5.addTextlabel("Quadro","Quadro: "+(windowSpan*1000)/20000 + " ms",70,420); 
        
        Knob_zoom = controlP5.addKnob("Zoom").setRange(0,100).setValue(0).setPosition(1,100).setRadius(35).setDragDirection(Knob.HORIZONTAL);
...
void Zoom(){
        float x = (((ScaleMaxGlobal - ScaleMinGlobal)/2)*Knob_zoom.getValue())/100;//é calculado o zoom em porcentagem em cima do valor setado no globalscale        
        InScaleMin = ScaleMinGlobal + x;
        InScaleMax = ScaleMaxGlobal - x;
        println(x);
}
...
void Time(){
        ...
        nPoints = 0;
        pointWidth= (ioWidth+100)/float(arrayLength-1);//coloquei +50 no width
...
void serialEvent(Serial myPort){
  ...
  if (s.length == 8){
    ...
    if(!A0) InLabelA0.setValue("Channel 1 : "+float(s[0]));
    else InLabelA0.setValue("Channel 1 : disable");
    if(!A1) InLabelA1.setValue("Channel 2 : "+float(s[1]));
    else InLabelA1.setValue("Channel 2 : disable");
    if(!A2) InLabelA2.setValue("Channel 3 : "+float(s[2]));
    else InLabelA2.setValue("Channel 3 : disable");
    if(!A3) InLabelA3.setValue("Channel 4 : "+float(s[3]));
    else InLabelA3.setValue("Channel 4 : disable");
    if(!A4) InLabelA4.setValue("Channel 5 : "+float(s[4]));
    else InLabelA4.setValue("Channel 5 : disable");
    if(!A5) InLabelA5.setValue("Channel 6 : "+float(s[5]));
    else InLabelA5.setValue("Channel 6 : disable");
